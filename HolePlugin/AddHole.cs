...
namespace HolePlugin
{
    ...
    public class AddHole : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            LogicalOrFilter ductsPipesFilter = new LogicalOrFilter(ductsFilter, pipesFilter);
            List<Element> ductsPipesList = new FilteredElementCollector(ovDoc)
                .WherePasses(ductsPipesFilter)
                .ToElements()
                .ToList();
            ...
            transaction.Start("Расстановка");
            foreach (Element elem in ductsPipesList)
            {
                ...
                foreach (ReferenceWithContext refer in intersections)
                {
                    ...
                    {
                        width.Set((elem as Duct).Diameter);
                        height.Set((elem as Duct).Diameter);
                    }
                    if (elem.Category.Id.IntegerValue == (int)BuiltInCategory.OST_PipeCurves)
                    {
                        width.Set((elem as Pipe).Diameter);
                        height.Set((elem as Pipe).Diameter);
                    }
